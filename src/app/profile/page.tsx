...
import { setCache, isCacheStale } from "@/lib/cache";

const PROFILE_DATA_KEY = "lotc_profile_data";

function saveProfileData(data: Record<string, unknown>) {
  try {
    const existing = JSON.parse(localStorage.getItem(PROFILE_DATA_KEY) || "{}");
    localStorage.setItem(PROFILE_DATA_KEY, JSON.stringify({ ...existing, ...data, _ts: Date.now() }));
  } catch { /* ignore */ }
}

function loadProfileData<T>(key: string): T | null {
  try {
  ...
  useEffect(() => {
    ...
    async function fetchAll(sig: AbortSignal) {
      ...
      if (statsRes.status === "fulfilled" && statsRes.value.data) {
        ...
        saveProfileData({ solutionCount: solution_count, discussionCount: discussion_count });
      }
      ...
      if (detailsRes.status === "fulfilled" && detailsRes.value.data) {
        ...
        saveProfileData({ bookmarked, solved });
      }
      ...
      if (heatmapRes.status === "fulfilled" && heatmapRes.value.data && !heatmapRes.value.error) {
        ...
        saveProfileData({ dates: expanded });
      }
